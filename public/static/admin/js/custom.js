...
const cropperImage = ({
    ...
    const cropModal = $('#cropperModal');
    const imagePreview = document.querySelector('#modal-image-preview');
    const cropButton = document.querySelector('#crop-modal-button');

    let cropper;
    let currentInput = null;
    let currentPreview = null;

    const handleImageInputChange = (input) => {
        const file = input.files[0];
        if (!file) return;

        currentSelector = inputSelectors.find(sel => document.querySelector(sel) === input);
        currentInput = input;
        currentPreview = document.getElementById(previewTarget[currentSelector] || '');
    ...
    cropModal.on('shown.bs.modal', function () {
        ...
        cropper = new Cropper(imagePreview, {
            aspectRatio: parseFloat(ratio[currentSelector] || 1),
            viewMode: 1,
            autoCropArea: 1,
        });
    });
